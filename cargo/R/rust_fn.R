...
rust_fn <- function(..., dependencies=character(0), minimum_version="1.31.0", verbose=FALSE, cached=TRUE, longjmp=TRUE, invisible=FALSE) {
  ...
  rustlib_directory <- file.path(path,"rustlib")
  cargo_toml_file <- file.path(rustlib_directory,"Cargo.toml")
  toml <- c('[package]',sprintf('name = "%s"',libname),'version = "0.1.0"','edition = "2018"','')
  ...
  if ( is_mac ) {
    rustflags <- paste0(rustflags, ' -C link-args=-Wl,-undefined,dynamic_lookup')
  } else if ( is_windows ) {
    rustflags <- paste0(rustflags, sprintf(' -C link-arg=-L -C link-arg=%s -C link-arg=-lR', R.home("bin")))
  }
  # Write Rust code
  ...
  env_rust_flags <- Sys.getenv("RUSTFLAGS","<unset>")
  env_save_cache <- Sys.getenv("R_CARGO_SAVE_CACHE","<unset>")
  env_build_jobs <- Sys.getenv("R_CARGO_BUILD_JOBS","<unset>")
  env_func_dir   <- Sys.getenv("ROXIDO_R_FUNC_DIR","<unset>")
  Sys.setenv("RUSTFLAGS"=rustflags)
  Sys.setenv("R_CARGO_SAVE_CACHE"="TRUE")
  Sys.setenv("R_CARGO_BUILD_JOBS"="0")
  Sys.setenv("ROXIDO_R_FUNC_DIR"=r_code_directory)
  on.exit(add=TRUE, {
    if ( env_rust_flags != "<unset>" ) Sys.setenv("RUSTFLAGS"=env_rust_flags)
    if ( env_save_cache != "<unset>" ) Sys.setenv("R_CARGO_SAVE_CACHE"=env_save_cache)
    if ( env_build_jobs != "<unset>" ) Sys.setenv("R_CARGO_BUILD_JOBS"=env_build_jobs)
    if ( env_func_dir != "<unset>" ) Sys.setenv("ROXIDO_R_FUNC_DIR"=env_func_dir)
    setwd(cwd)
  })
  options <- if ( ! isTRUE(verbose) ) "--quiet" else character(0)
  if ( ! run(options, "build", "--release", verbose=isTRUE(verbose), minimum_version=minimum_version) ) stop("Couldn't build Rust code.")
...
get_lib_path <- function(verbose, cached) {
  ...
  if ( ! isTRUE(cached) || ! file.exists(stamp_file) || cargo_version > readRDS(stamp_file) ) {
    ...
    file.copy(system.file(file.path("template","src","rustlib","roxido"), package="cargo"), rustlib_directory, recursive=TRUE)
    file.copy(system.file(file.path("template","src","rustlib","roxido_macro"), package="cargo"), rustlib_directory, recursive=TRUE)
    unlink(file.path(path, "rustlib", "target"),  recursive=TRUE, force=TRUE)
  }
